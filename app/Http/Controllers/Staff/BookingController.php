...
class BookingController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('staff.payments.page', $booking->id);
    }

    public function payment(Booking $booking)
    {
        return view('staff.payments.payment', [
    ...
    public function paymentTourCallback(Request $request)
    {
        ...
        if (!$transactionId || !$txRef) {
            return redirect()->route('staff.bookings.index')->with('error', 'Invalid payment callback data.');
        }
        ...
        if (!$booking) {
            return redirect()->route('staff.bookings.index')->with('error', 'Booking not found.');
        }

        // Prevent duplicate payment processing
        if ($booking->status === 'paid') {
            return redirect()->route('staff.bookings.index')->with('success', 'Payment already processed.');
        }
        ...
        if ($response->failed()) {
            ...
            return redirect()->route('staff.bookings.index')->with('error', 'Could not verify payment. Please contact support.');
        }
        ...
        if (isset($data['status']) && $data['status'] === 'successful') {
            ...
            return redirect()->route('staff.bookings.index')->with('success', 'Payment successful and booking confirmed!');
        }

        // If payment was cancelled or failed
        return redirect()->route('staff.bookings.index')->with('error', 'Payment was not successful.');
    }
